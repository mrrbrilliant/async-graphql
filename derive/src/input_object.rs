use crate::args;
use crate::utils::{generate_default, generate_validator, get_crate_name, get_rustdoc};
...
pub fn generate(object_args: &args::InputObject) -> Result<TokenStream> {
    ...
    let s = match &object_args.data {
        ...
        _ => {
            return Err(Error::new_spanned(
                &object_args.data,
                "InputObject can only be applied to an struct.",
            ))
        }
    };
    ...
    for field in &s.fields {
        ...
        let validator = generate_validator(&crate_name, &*field.validator)?;
        let desc = get_rustdoc(&field.attrs)?
            .map(|s| quote! { Some(#s) })
            .unwrap_or_else(|| quote! {None});
        let default = generate_default(field.default, &*field.default_value, &*field.default_with)?;
